...
from src.controllers.revenue import (
    fetch_revenue,
    insert_revenue,
    update_revenue_controller,
)
...
@revenue_router.post("")
async def creat_revenue(
    request: Request, body: Revenue, pg_db_session: Session = Depends(get_db)
):
    try:
        user_id = request.state.user_id
        role = request.state.role
        return insert_revenue(
            user_id=user_id, user_role=role, data=vars(body), db=pg_db_session
        )
...
def update_revenue_route(
    ...
    try:
        ...
        return update_revenue_controller(
            revenue_id=revenue_id, data=data, user_id=user_id, role=role, db=db
        )
    except HTTPException as e:
        raise e
