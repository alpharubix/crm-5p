...
class DealTask(Base):
    ...
    module_name = Column(String, default="Deal", nullable=False)
    ...
    target_deal_status = Column(String(100), nullable=True)
    ...
    # Relationships
    deal = relationship("Deal", backref="deal_tasks")
    assigned_to = relationship(
        "User", foreign_keys=[assigned_to_id], backref="assigned_deal_tasks"
    )
    created_by = relationship(
        "User", foreign_keys=[created_by_id], backref="created_deal_tasks"
    )
    modified_by = relationship(
        "User", foreign_keys=[modified_by_id], backref="modified_deal_tasks"
    )

    @hybrid_property
    def deal_name(self):
        return self.deal.deal_name if self.deal else None

    @hybrid_property
    def account_name(self):
        return self.deal.account_name if self.deal else None

    @hybrid_property
    def account_id(self):
        return self.deal.account_id if self.deal else None

    @hybrid_property
    def deal_status(self):
        return self.deal.deal_status if self.deal else None

    @hybrid_property
    def deal_stage(self):
        return self.deal.deal_stage if self.deal else None

    @hybrid_property
    def deal_owner(self):
        if self.deal and self.deal.owner:
            return self.deal.owner.full_name or self.deal.owner.email
        return None

    @hybrid_property
    def deal_owner_id(self):
        return self.deal.deal_owner_id if self.deal else None

    @hybrid_property
    def loan_type(self):
        return self.deal.loan_type if self.deal else None

    @hybrid_property
    def lender_name(self):
        return self.deal.lender_name if self.deal else None

    @hybrid_property
    def call_back_date_status(self):
        if not self.deal or not self.deal.deal_call_back_datetime:
            return "Blank"
        status_name, _ = get_call_back_info(self.deal.deal_call_back_datetime)
        return status_name
