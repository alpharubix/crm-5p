    ...
    if not created_by_name and task.created_by_id:
        ...
        if users_map and c_str in users_map:
            ...
            try:
                u = (
                    db.query(User)
                    .filter(or_(User.id == int(task.created_by_id), User.zuid == c_str))
                    .first()
                )
                if u:
    ...
    if not assigned_name and assigned_id:
        ...
        if users_map and a_str in users_map:
            ...
            try:
                u = (
                    db.query(User)
                    .filter(or_(User.id == int(assigned_id), User.zuid == a_str))
                    .first()
                )
                if u:
...
def update_account_task(
    ...
    task = (
        ...
        .filter(AccountTask.id == task_id, AccountTask.company_id == 2)
        .first()
    )
    if not task:
        raise HTTPException(
    ...
    if new_requested_status:
        ...
        if not is_owner and role not in ("super_admin", "admin", "manager") and current_user_id not in bypass_ids:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    for field, val in update_data.items():
        if field == "completed_at" and val is None and new_requested_status == "Completed":
            continue
...
def bulk_update_task_status(
    ...
    if role not in ("super_admin", "admin") and (
        ...
        unauthorized_tasks = [
            t.id for t in tasks if str(t.created_by_id) != str(current_user_id)
        ]
