...
def notify_task_unassigned_status_change(
    ...
    when a task status changes from 'Unassigned' to any other status in 5PointCredit.
    """
    ...
    try:
        ...
        <p>Please log in to the 5PointCredit CRM system to view full details.</p>
        <p>Regards,<br>5PointCredit CRM System</p>
        """
...
def notify_deal_task_unassigned_status_change(
    ...
    when a deal task status changes from 'Unassigned' to any other status in 5PointCredit.
    """
    from sqlalchemy.orm import joinedload
