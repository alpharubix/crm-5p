    ...
    if account_owner_id:
        ...
        if owner_ids:
            if role in ("super_admin", "admin"):
                ...
                allowed_owner_ids = [user_id] + MANAGER_EXECUTIVES_MAP.get(user_id, []) if role == "manager" else [user_id]
                allowed_set = {int(x) for x in allowed_owner_ids}
...
def export_deals_csv(
    ...
    if case_status:
        ...
        if statuses:
            filters.append(
                or_(*[Deal.case_status.ilike(f"%{s}%") for s in statuses])
            )
    if ticket_login:
